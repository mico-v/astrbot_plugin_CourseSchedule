...
- 在 WebUI 中导出 / 导入：整个群组可导出为 `.ics` 压缩包或原始数据备份，单个成员可导出为
  `schedule<QQ号>.ics`，也可以导入这两种压缩包和单个 `.ics` 还原课表。
...
在 WebUI 的「课表管理」页面，每个群组右侧的 `⇅` 按钮会打开该群组的导入 / 导出对话框：

- **导出 ICS 压缩包**：导出该群组全部成员，每位成员一个 `schedule<QQ号>.ics` 文件，压缩包里
  另有 `manifest.json`（导出时间、来源会话、成员名单）和 `README.txt`。文件名沿用群文件约定，
  所以解开压缩包后既可以把 `.ics` 直接导入手机或电脑日历，也可以把某个文件发到群里用
  `/导入课表` 还原给对应成员。
- **导出原始备份**：按插件原本的存储方式导出，`members/schedule<QQ号>.json` 是每位成员的存储
  字段和事件原文（含 `RDATE`/`EXDATE` 等未建模属性），`day_overrides.json` 是休假/调休标记。
  用于把课表完整还原到同一个或另一个会话。
- **导入**：`.zip` 按压缩包里的成员 QQ 号导入到当前会话，包内没有的成员不受影响；导入原始
  备份会把包内成员的课程和休假/调休标记恢复成备份时的状态（包内没有的标记会被清除）。
  `.ics` 导入到当前选中的成员，文件名为 `schedule<QQ号>.ics` 时以文件名里的 QQ 号为准。
  两种导入都不会覆盖已有成员的昵称。

成员编辑区右上角的 `导出 .ics` 按钮单独导出当前成员，文件名同样是 `schedule<QQ号>.ics`，
文件内的日历名称（`X-WR-CALNAME`）写成「<昵称>的课表」，导入日历后就能看到是谁的课表。

ICS 压缩包只有日历事件，不含休假/调休标记；需要连标记一起还原时请使用原始备份。导入时只在
内存里解压，不会按条目名写磁盘：单个 `.ics` 上限 2 MiB（与聊天导入一致），压缩包上限 16 MiB、
解压后 32 MiB、最多 500 个条目，无法识别成员的文件名会被跳过并在结果里说明。
