...
**Day-image layout.** `domain.split_folded_rows` splits `daily_member_rows` into `(cards, folded)`: any member with no class left on the day (`finished`/`none`/`holiday`) is drawn as a compact avatar grid by `render._draw_folded_members` instead of a full card, so the cards always mean "still has something coming". The rule depends only on `status_key`, not on whether the day is today — viewing a quiet tomorrow folds its idle members the same way. A consequence worth remembering: a *past* day is entirely finished, so everyone folds into the strip. The folded strip counts toward the subtitle total, the legend only lists statuses a card actually shows, and the strip title names the day being viewed.
